...
namespace DfE.GIAP.Web.Tests.Controllers.Admin
{
    ...
    public class ManageDocumentsControllerTests : IClassFixture<UserClaimsPrincipalFake>, IClassFixture<ManageDocumentsResultsFake>
    {
        ...
        private readonly Mock<IManageDocumentsService> _mockDocRepo = new Mock<IManageDocumentsService>();
        private readonly Mock<INewsService> _mockNewsService = new Mock<INewsService>();
        ...
        {
            // Arrange
            var mockDocRepo = new Mock<IManageDocumentsService>();
            var mockNewsService = new Mock<INewsService>();
            CommonResponseBody commonResponseBody = new CommonResponseBody() { Title = "testTitle", Body = "testBody", Id = "NewsArticle", Published = false };
            ...
            var _mockLogger = Substitute.For<ILogger<ManageDocumentsController>>();
            var _commonService = new Mock<ICommonService>();
            var _mockCookieManager = new Mock<ICookieManager>();

            var controller = new ManageDocumentsController(mockNewsService.Object, mockDocRepo.Object, _mockContentService.Object);

            // Act
            var saveAsDraft = "SaveAsDraft";
            var result = await controller.SaveArticleAsDraft(model, saveAsDraft).ConfigureAwait(false);
        ...
        public async Task Check_news_article_is_deleted()
        {
            ...
            // Act
            var result = await controller.DeleteNews(model).ConfigureAwait(false);
        ...
        public void CreateNewsArticle_RendersView()
        {
            ...
            // Act
            var result = controller.CreateNewsArticle();
            // Assert
            Assert.IsType<ViewResult>(result);
        }
        ...
        public async Task EditNewsArticle_RendersView_With_Values()
        {
            ...
            _mockNewsService.Setup(repo => repo.GetNewsArticleById(It.IsAny<string>())).ReturnsAsync(article);

            var controller = GetManageDocumentsController();
            var editDocument = "EditDocument";

            // Act
            var result = await controller.EditNewsArticle(model, editDocument).ConfigureAwait(false);
        ...
        public async Task PublishNewsArticle_Sets_ActionType_As_Published()
        {
            ...
            var publish = "Publish";
            // Act
            var result = await controller.PublishNewsArticle(model, publish).ConfigureAwait(false);
            // Assert
            var viewResult = Assert.IsType<ViewResult>(result);
        }
        ...
        public async Task PreviewNewsArticle_With_Values()
        {
            ...
            mockContentService.Setup(repo => repo.AddOrUpdateDocument(It.IsAny<CommonRequestBody>(), It.IsAny<AzureFunctionHeaderDetails>())).ReturnsAsync(responseBody);
            var mockDocRepo = new Mock<IManageDocumentsService>();
            var mockNewsService = new Mock<INewsService>();
        ...
        public async Task PreviewNewsArticle_When_Article_Is_Pinned()
        {
            ...
            var mockContentService = new Mock<IContentService>();
            mockContentService.Setup(repo => repo.AddOrUpdateDocument(It.IsAny<CommonRequestBody>(), It.IsAny<AzureFunctionHeaderDetails>())).ReturnsAsync(responseBody);
            mockContentService.Setup(repo => repo.SetDocumentToPublished(It.IsAny<CommonRequestBody>(), It.IsAny<AzureFunctionHeaderDetails>())).ReturnsAsync(responseBody);
            var mockDocRepo = new Mock<IManageDocumentsService>();
            var mockNewsService = new Mock<INewsService>();
        ...
        {
            return new ManageDocumentsController(_mockNewsService.Object, _mockDocRepo.Object, _mockContentService.Object);
        }
    }
}