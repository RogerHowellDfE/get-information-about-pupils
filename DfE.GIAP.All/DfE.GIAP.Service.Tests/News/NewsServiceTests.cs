...
namespace DfE.GIAP.Service.Tests.Services.News
{
    ...
    public class NewsServiceTests
    {
        ...
        [Fact]
        public async Task GetNewsArticleById()
        {
            // arrange
            var article = new Article() { Id = "2222abcd-1a2b-1111-1234-abcd1234efgh", Body = "Test body 2" };
            var requestBody = new RequestBody();

            var fakeHttpRequestSender = new Mock<IFakeHttpRequestSender>();
            var fakeHttpMessageHandler = new FakeHttpMessageHandler(fakeHttpRequestSender.Object);
            var httpClient = new HttpClient(fakeHttpMessageHandler);
            var httpResponse = new HttpResponseMessage { Content = new StringContent(JsonConvert.SerializeObject(article)) };
            fakeHttpRequestSender.Setup(x => x.Send(It.IsAny<HttpRequestMessage>())).Returns(httpResponse);

            var apiProcessorService = new ApiService(httpClient, null);

            var url = "https://www.somewhere.com?code=1234";
            var urls = new AzureAppSettings() { QueryNewsArticleUrl = url };
            ...
            // act
            var actual = await newsService.GetNewsArticleById(article.Id);

            // assert
            fakeHttpRequestSender.Verify(x => x.Send(It.IsAny<HttpRequestMessage>()), Times.Once());

            Assert.IsType<Article>(article);
            Assert.Equal("2222abcd-1a2b-1111-1234-abcd1234efgh", article.Id);
            Assert.Equal("Test body 2", article.Body);
        }
