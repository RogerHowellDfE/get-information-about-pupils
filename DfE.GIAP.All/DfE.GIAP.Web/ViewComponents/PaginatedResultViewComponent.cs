...
namespace DfE.GIAP.Web.ViewComponents
{
    public class PaginatedResultViewComponent : ViewComponent
    {
        ...
        {
            var numberOfPages = GetNumberOfPages(pageSize, total);
        ...
        private int GetNumberOfPages(int pageSize, long total)
        {
            if (total > 100000) return Convert.ToInt32(Math.Ceiling((decimal)100000 / (decimal)pageSize));
            return Convert.ToInt32(Math.Ceiling((decimal)total / (decimal)pageSize));
        }
